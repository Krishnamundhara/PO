...
/**
 * POButton - A fancy animated gradient button with icon support
 * @param {Object} props
 * @param {React.ReactElement} props.icon - Lucide icon element
 * @param {string} props.title - Main title text
 * @param {string} [props.subtitle] - Optional subtitle text
 * @param {'sm' | 'md' | 'lg'} [props.size='md'] - Button size
 * @param {Object} [props.POButtonLight] - Light mode gradient colors
 * @param {Object} [props.POButtonDark] - Dark mode gradient colors
 */
export default function POButton({
  ...
  POButtonLight = { from: "from-indigo-700", via: "via-black/80", to: "to-black/100" },
  POButtonDark = { from: "from-indigo-800/30", via: "via-black/50", to: "to-black/70" },
  ...props
}) {
  const sizes = {
    sm: "p-2 rounded-xl",
  ...
  return (
    <button
      ...
      className={`group relative overflow-hidden border-2 cursor-pointer transition-all duration-500 ease-out 
                  ...
                  ${sizes[size]} 
                  border-indigo-400/60 bg-gradient-to-br ${POButtonLight.from} ${POButtonLight.via} ${POButtonLight.to} 
